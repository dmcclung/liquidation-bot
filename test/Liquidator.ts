...
describe("Liquidator", () => {
  ...
  const printAccountLimits = async () => {
    ...
    let accountLimits = await joelens.callStatic.getAccountLimits(joetrollerAddress, accountId)
  ...
  const printAvaxPrice = async () => {
    ...
    let avaxPrice = await priceOracle.callStatic.getUnderlyingPrice(tokenId)
  ...
  const printCollateralFactor = async () => {
    ...
    let market = await joetroller.markets(tokenId)
  ...
  it("should liquidate an unhealthy account", async () => {
    ...
    const adminJoetroller = await ethers.getContractAt(joetrollerAbi, joetrollerAddress, adminSigner)

    const tx = await adminJoetroller._setCollateralFactor(tokenId, ethers.BigNumber.from('550000000000000000'))
    await tx.wait()
